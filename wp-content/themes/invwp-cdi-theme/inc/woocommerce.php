 ...
 * @return void
 */
function invwp_woocommerce_setup() {
	add_theme_support(
		'woocommerce',
		array(
...
add_action( 'after_setup_theme', 'invwp_woocommerce_setup' );
 ...
 * @return void
 */
function invwp_woocommerce_scripts() {
	wp_enqueue_style( 'invwp-woocommerce-style', get_template_directory_uri() . '/woocommerce.css', array(), _S_VERSION );
	...
	wp_add_inline_style( 'invwp-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'invwp_woocommerce_scripts' );
 ...
 */
function invwp_woocommerce_active_body_class( $classes ) {
	$classes[] = 'woocommerce-active';

	return $classes;
}
add_filter( 'body_class', 'invwp_woocommerce_active_body_class' );
 ...
 */
function invwp_woocommerce_related_products_args( $args ) {
	$defaults = array(
		'posts_per_page' => 3,
		'columns'        => 3,
	);
...
add_filter( 'woocommerce_output_related_products_args', 'invwp_woocommerce_related_products_args' );
...
if ( ! function_exists( 'invwp_woocommerce_wrapper_before' ) ) {
	/**
	 * Before Content.
	 *
...
if ( ! function_exists( 'invwp_woocommerce_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
...
if ( ! function_exists( 'invwp_woocommerce_header_cart' ) ) {
	 ...
	 * @return void
	 */
	function invwp_woocommerce_header_cart() {
		if ( is_cart() ) {
