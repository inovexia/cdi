...
class Permalink_Manager_Pro_Functions extends Permalink_Manager_Class {
	...
	public static function get_expiration_date($basic_check = false, $empty_if_valid = false, $update_available = true) {
		...
		// License key is abused
			...
			$expiration_info = sprintf(__('This license is already in use on another website and cannot be used to request automatic update for this domain.', 'permalink-manager'), $license_info_page) . " ";
	...
	function license_info_bar($plugin_file, $plugin_data) {
		...
		$exp_info_text = self::get_expiration_date(false, true, $update_available);

		if($exp_info_text) {
			printf('<tr class="plugin-update-tr active" data-slug="%s" data-plugin="%s"><td colspan="%d" class="plugin-update colspanchange plugin_license_info_row">', $plugin_data['slug'], $plugin_file, $column_count);
	...
	function pro_alerts($alerts = array()) {
		...
		$exp_info_code = self::get_expiration_date(true, true);
