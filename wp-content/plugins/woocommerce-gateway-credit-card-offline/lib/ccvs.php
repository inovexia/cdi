...
class CreditCardValidationSolution {
    ...
    function validateCreditCard($Number, $Language='en', $Accepted='',
        ...
        include apply_filters( 'offline_cc_ccvs_language_file', "$Path/language/ccvs_$Language.inc" );
        ...
        $this->CCVSBIN         = substr($this->CCVSNumber, 0, 6);
        ...
        } elseif (($this->CCVSNumberLeft >= 2221) && ($this->CCVSNumberLeft <= 2720)) {
            $this->CCVSType = 'MasterCard';
            $ShouldLength = 16;
        ...
        // Maestro support
        // (based on jquery.payment.js - patterns: 5018, 502, 503, 506, 56, 58, 639, 6220, 67)
        } elseif ($this->CCVSNumberLeft == 5018) {
            $this->CCVSType = 'Maestro';
            if ( $NumberLength < 12 || $NumberLength > 19 ) {
                $ShouldLength = 16;             // trigger length error
            } else {
                $ShouldLength = $NumberLength;  // accept any length between 12 and 19 digits
            }

        } elseif (($this->CCVSNumberLeft >= 5020) && ($this->CCVSNumberLeft <= 5029)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 5030) && ($this->CCVSNumberLeft <= 5039)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 5060) && ($this->CCVSNumberLeft <= 5069)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 5600) && ($this->CCVSNumberLeft <= 5699)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 5800) && ($this->CCVSNumberLeft <= 5899)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 6390) && ($this->CCVSNumberLeft <= 6399)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif ($this->CCVSNumberLeft == 6220) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits

        } elseif (($this->CCVSNumberLeft >= 6700) && ($this->CCVSNumberLeft <= 6799)) {
            $this->CCVSType = 'Maestro';
            $ShouldLength = ( $NumberLength < 12 || $NumberLength > 19 ) ? 16 : $NumberLength; // accept any length between 12 and 19 digits
